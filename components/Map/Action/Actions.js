...
export default function Action({ title, meta, intro, slug }) {
  const i18n = useI18n();
  ...
  return (
    <NextLink href={`/${i18n.t('action.slug')}/${slug}`}>
      <a className="block bg-turquoise-300 hover:bg-black hover:text-white p-10 h-full">
        {meta && (
          <span className="block font-rubik text-2xs italic">{meta}</span>
        )}
        ...
        {intro && <p className="block font-rubik text-2xs mt-5">{intro}</p>}
      </a>
    </NextLink>
  );
}
