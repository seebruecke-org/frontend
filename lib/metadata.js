...
export function mergeMetadata(defaults, data) {
  ...
  // lambda
  if (defaultData['twitter_image']) {
    defaultData['twitter_image'] = getFullClientUrl(
      `/api/resize/?url=${getFullCMSUrl(defaultData['twitter_image'])}`
    );
  }

  if (defaultData['facebook_image']) {
    defaultData['facebook_image'] = getFullClientUrl(
      `/api/resize/?url=${getFullCMSUrl(defaultData['facebook_image'])}`
    );
  }
