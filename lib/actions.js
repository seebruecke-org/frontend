...
export function formatStartAndEnd(start, end, format) {
  ...
  const formattedStart =
    start && format(startDate, format.dateTime, { locale: de });

  const formattedEnd =
    end &&
    format(endDate, `${startEndIsSameDay ? '' : format.date} ${format.time}`, {
      locale: de
    });

  return `${formattedStart} — ${formattedEnd} ${format.timePostfix}`;
}
...
export async function fetchAllActions(locale, format) {
  ...
  return actions.reduce((acc, action) => {
    ...
    const key = format(getDateWithFixedTZ(start), format.date);
    ...
    acc[key].push({
      ...action,
      start: formatTime(start, format.dateTime),
      end: formatTime(end, format.dateTime),
...
export async function fetchActionBySlug(slug, locale, format) {
