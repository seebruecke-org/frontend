...
export function formatStartAndEnd(start, end, formatting) {
  ...
  const formattedStart = format(startDate, formatting.dateTime);
  ...
  const formattedEnd = format(endDate, startEndIsSameDay ? formatting.time : formatting.dateTime);
...
export async function fetchAllActions(locale, formatting, options) {
  ...
  return actions.reduce((acc, { start, end, coordinates, ...action }) => {
    ...
    acc[key].push({
      ...
      starttime: formatTime(start, formatting.time),
      endtime: formatTime(end, formatting.time),
