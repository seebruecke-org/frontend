...
export function formatStartAndEnd(
  ...
  { dateFormat, timeFormat, dateTimeFormat, timePostfix }
) {
  ...
  const formattedStart =
    start && format(startDate, dateTimeFormat, { locale: de });

  const formattedEnd =
    end &&
    format(endDate, `${startEndIsSameDay ? '' : dateFormat} ${timeFormat}`, {
      locale: de
    });

  return `${formattedStart} — ${formattedEnd} ${timePostfix}`;
}
...
export function formatTime(date, dateFormat) {
  ...
  return format(getDateWithFixedTZ(date), dateFormat, {
    locale: de
  });
}
...
export async function fetchAllActions(locale, translations) {
  ...
  return actions.reduce((acc, action) => {
    ...
    const key = format(getDateWithFixedTZ(start), translations.dateFormat);
    ...
    acc[key].push({
      ...action,
      start: formatTime(start, translations.dateTimeFormat),
      end: formatTime(end, translations.dateTimeFormat),
...
export async function fetchActionBySlug(slug, locale, translations) {
