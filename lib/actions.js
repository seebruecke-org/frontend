...
export async function fetchActionBySlug(slug, locale, formatting, options) {
  async function fetchData(locale) {
    const { actions } = await fetchAPI(
      `
    query ActionBySlug {
      actions(where: { slug: "${slug}"}, locale: "${locale}") {
        title
        intro
        start
        end
  ...
  let actions = await fetchData(locale);
  if (actions.length === 0){
    actions = await fetchData("de");
  }
  // console.log({actions: actions});
  const action = actions && actions[0];

  if (action && action?.group?.city?.name) {
    action.group.city.uri = await createCityUri(action.group.city.slug, {locale:locale, dePlanB: true});
  }
