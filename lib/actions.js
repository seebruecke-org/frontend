...
export function formatStartAndEnd(start, hideStartTime, end, hideEndTime, formatting) {
  const startDate = getDateWithFixedTZ(start);
  const formattedStart = format(startDate, hideStartTime ? formatting.date : formatting.dateTime);

  if (!end)
    return `${formattedStart} ${formatting.timePostfix}`;

  const endDate = getDateWithFixedTZ(end);
  let endDateFormat = formatting.dateTime;
  const startEndSameDate = isSameDay(startDate, endDate);

  if (startEndSameDate && hideEndTime) {
    return `${formattedStart} ${formatting.timePostfix}`;
  }

  if (startEndSameDate && !hideStartTime) {
    endDateFormat = formatting.time
  }

  if (hideEndTime) {
    endDateFormat = formatting.date;
  }

  const formattedEnd = format(endDate, endDateFormat);
...
export async function fetchAllActions(locale, formatting, options) {
  ...
  return actions.reduce((acc, { start, coordinates, hide_start_time, ...action }) => {
    ...
    acc[key].push({
      ...
      starttime: hide_start_time ? null : formatTime(start, formatting.time),
...
export async function fetchActionBySlug(slug, locale, formatting, options) {
  ...
  if (action) {
    ...
    action.formattedDateRange = formatStartAndEnd(
      action.start,
      action.hide_start_time,
      action.end,
      action.hide_end_time,
      formatting
    );
  }
...
export async function fetchLatestActionPaths(locale, options) {
  const { actions } = await fetchAPI(
    `
    query {
