...
export async function getPage(slug, parent, locale, formatting) {
  const { pages } = await fetchAPI(`
    query {
  ...
  return await sideloadBlockData(page, 'content', formatting);
}

async function queryAPI(slug = [], locale, formatting) {
  ...
  const page = await getPage(lastSlug, parents, locale, formatting);
...
export async function query(slug, locale, formatting) {
  return {
    data: await queryAPI(slug, locale, formatting)
  };
}
