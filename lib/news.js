...
export async function fetchRecentNews(options = {}, format) {
  ...
  return Promise.all(
    news.map(async (entry) => {
      ...
      return {
        ...entry,
        publication_date: format(new Date(publication_date), format.date),
        excerpt
      };
    })
  );
...
export async function fetchNewsBySlug(slug, locale, format) {
  const { news } = await fetchAPI(`
    query {
  ...
  if (entry?.published_at) {
    entry.published_at = format(new Date(entry.published_at), format.date);
  }
...
export async function fetchAllNewsPaths(locale) {
  const { news } = await fetchAPI(`
    query {
      news: newsEntries {
        slug
