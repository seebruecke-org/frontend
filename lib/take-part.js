...
async function fetchSafeHarbour(citySlug) {
  const translations = await import('@/locales/de/common.json');
  const { cities } = await fetchAPI(`
    query {
      cities(where: { slug: "${citySlug}"}) {
        name
  ...
  if (city?.safe_harbour?.since) {
    city.safe_harbour.since = format(
      ...
      translations.safeHarbour.dateFormat
    );
  }
    ...
    if (federalCountryExists) {
      ...
      if (federalCountryHasContent) {
        acc[countryName].countries[federalCountryName].uri =
          await createFederalCountryUri(federal_country.slug);
      }
    }
...
export async function fetchAllSafeHarbours() {
  ...
  const citiesWithSafeHarbour = cities
    ...
    .map(({ safe_harbour, ...city }) => {
      const { demands, since } = safe_harbour;
      ...
      return {
        ...city,
        since:
          since && format(new Date(since), translations.safeHarbour.dateFormat),
        safe_harbour
      };
    });
...
async function fetchData(slug = []) {
  switch (slug.length) {
    ...
    case 3: {
      ...
      const city = await fetchGroup(citySlug);
      let breadcrumbs = await createCityBreadcrumbs(citySlug);
      let navigation = await createCityNavigation(citySlug);
      ...
      const safeHarbour = await fetchSafeHarbour(citySlug);
      breadcrumbs = await createCityBreadcrumbs(citySlug);
      navigation = await createCityNavigation(citySlug);
