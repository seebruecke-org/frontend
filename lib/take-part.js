...
import FRAGMENT_SAFE_HARBOURS from '@/components/Demands/SafeHarbour';
...
async function fetchSafeHarbour(citySlug) {
  const translations = await import('@/locales/de/safe-harbour.json');
  const { cities } = await fetchAPI(`
    query {
      cities(where: { slug: "${citySlug}"}) {
        name
...
export async function fetchAllSafeHarbours() {
  ...
  const citiesWithSafeHarbour = cities
    ...
    .map(({ safe_harbour, ...city }) => {
      ...
      safe_harbour.since =
        safe_harbour?.since &&
        format(new Date(safe_harbour.since), translations.dateFormat);
