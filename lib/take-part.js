...
async function fetchSafeHarbour(citySlug, format) {
  const { cities } = await fetchAPI(`
...
export async function fetchAllSafeHarbours(locale, format) {
  const cities = await fetchAllCities(locale);
  const citiesWithSafeHarbour = cities
    ...
    .map(({ safe_harbour, ...city }) => {
      ...
      safe_harbour.since =
        safe_harbour?.since &&
        format(new Date(safe_harbour.since), format.date);
...
async function fetchData(slug = [], locale, format) {
  switch (slug.length) {
    // /[country]/
    case 1: {
