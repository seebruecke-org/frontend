...
async function fetchGroup(citySlug, formatting) {
  const { cities } = await fetchAPI(`
    query GroupBySlug {
  ...
  city.group = await sideloadBlockData(city.group, 'content', formatting);
  city.group.headlines = await createGroupNavigation(city.group.content);

  return city;
}

async function fetchCountry(countrySlug, formatting) {
  const { countries } = await fetchAPI(`
    query CountryBySlug {
  ...
  return await sideloadBlockData(country, 'content', formatting);
}

async function fetchFederalCountry(countrySlug, formatting) {
  ...
  return await sideloadBlockData(federalCountry, 'content', formatting);
}

async function fetchSafeHarbour(citySlug, locale, formatting) {
  const { cities } = await fetchAPI(`
...
async function fetchData(slug = [], locale, formatting) {
  switch (slug.length) {
    ...
    case 2: {
      ...
      const city = await fetchGroup(lastSlug, locale, formatting);
    ...
    case 3: {
      ...
      const city = await fetchGroup(citySlug, locale, formatting);
      ...
      const safeHarbour = await fetchSafeHarbour(citySlug, locale, formatting);
    ...
    case 4: {
      ...
      const city = await fetchSafeHarbour(citySlug, locale, formatting);
...
export async function query(slug, locale, formatting) {
  const data = await fetchData(slug, locale, formatting);
