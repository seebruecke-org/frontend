...
export async function fetchAllSafeHarbours(locale) {
  const translations = await import('@/locales/de/safe-harbour.json');
  const cities = await fetchAllCities(locale);
...
async function fetchData(slug = [], locale) {
  switch (slug.length) {
    ...
    case 3: {
      ...
      const safeHarbour = await fetchSafeHarbour(citySlug, locale);
    ...
    case 4: {
      ...
      const city = await fetchSafeHarbour(citySlug, locale);
